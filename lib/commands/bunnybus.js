...
if (Args.publish) {
    ...
    const bunnyBusPublisher = new BunnyBusPublisher({ bunnyBus : config, tee : Args.tee ? true : false }, logger);
...
if (Args.subscribe) {
    ...
    const bunnyBusSubscriber = new BunnyBusSubscriber({ bunnyBus : config , durationMs : Args.duration }, logger);
...
if (Args.get) {
    ...
    const bunnyBusGet = new BunnyBusGet({ bunnyBus : config }, logger);
    const newLineTransform = new NewLineTransform();
